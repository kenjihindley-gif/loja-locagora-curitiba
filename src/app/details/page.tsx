...
function MotorcycleDetailsContent() {
  ...
  const handleOpenReservation = () => {
    ...
    if (needsSelection) {
        ...
        setReservationStep(savedSellerCode ? 3 : 2);
    }
  ...
  useEffect(() => {
    ...
    if (isReservationOpen && reservationStep === 3) {
      if (countdown > 0) {
      ...
      <Dialog open={isReservationOpen} onOpenChange={setIsReservationOpen}>
        <DialogContent className="sm:max-w-md">
          <DialogHeader>
            ...
            <DialogDescription>
              {reservationStep === 1 && "Escolha a cor e o plano desejado."}
              {reservationStep === 2 && "Se você possui um código de vendedor, insira abaixo."}
              {reservationStep === 3 && "Redirecionando para o WhatsApp..."}
            </DialogDescription>
          </DialogHeader>
          ...
          {reservationStep === 2 && (
            <div className="space-y-4 py-4">
              <div className="space-y-2">
                <Label htmlFor="seller-code" className="text-base font-semibold">Foi atendido por alguém? Digite o código:</Label>
                <Input 
                  id="seller-code" 
                ...
                />
                <span className="text-xs text-primary underline cursor-pointer mt-2 block opacity-70 hover:opacity-100 transition-opacity">
          ...
          {reservationStep === 3 && (
            <div className="space-y-6 py-4 text-center">
              <p className="text-sm font-medium text-destructive">
                Não apague nada escrito da mensagem automática, pois ela tem todas informações necessárias para finalizar a locação.
              </p>
              <div className="space-y-2">
                <Progress value={countdown * 10} className="w-full" />
                <p className="text-xs text-muted-foreground">Redirecionando em {countdown} segundos...</p>
              </div>
            </div>
          )}
          ...
          <DialogFooter className="flex-col sm:flex-row gap-2">
            ...
            {reservationStep === 2 && (
              <Button onClick={handleNextStep} className="w-full sm:w-auto">Confirmar</Button>
            )}
            {reservationStep === 3 && (
              <Button onClick={() => { window.location.href = generateWhatsAppLink(); setIsReservationOpen(false); }} className="w-full sm:w-auto">
                {getSellerInfo(sellerCode).hasCode 
                  ? `Ir para conversa com ${getSellerInfo(sellerCode).name} e Finalizar` 
                  : "OK, ir para o WhatsApp"}
              </Button>
            )}
          </DialogFooter>
        </DialogContent>
